...
export default function RegisterPage() {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    if (isLibrarian) {
      if (!formData.librarianCode) {
        setError('Please enter the librarian access code.');
        return;
      }
      setError('');
      setStep(2);
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center px-4 py-12 transition-colors duration-200 ${mounted && dark ? 'bg-slate-900' : 'bg-gradient-to-br from-indigo-50 via-white to-slate-100'} relative`}
    >
      {/* Dark mode toggle */}
      ...
      <div className={`w-full max-w-md rounded-2xl border shadow-xl px-8 py-10 ${card}`}>
        ...
        {success ? (
          <div className="text-center py-6">
            ...
            <p
              className={`text-sm font-mono font-bold mb-8 px-3 py-1.5 rounded-lg inline-block ${dark ? 'bg-slate-700 text-white' : 'bg-slate-100 text-slate-900'}`}
            >
              {createdUsername}
            </p>
            <Link
              href="/login"
              className="inline-flex w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-xl transition-colors items-center justify-center"
            >
              Go to Login
            </Link>
          </div>
          ...
          <>
            <h1
              className={`text-2xl font-bold text-center mb-1 ${dark ? 'text-white' : 'text-slate-900'}`}
            >
              Create Account
            </h1>
            ...
            {error && (
              <div className="mb-5 flex items-center gap-2 bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg px-4 py-3">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor">
                  <path d="M7 1a6 6 0 100 12A6 6 0 007 1zm0 9a.75.75 0 110-1.5.75.75 0 010 1.5zm.75-3.75a.75.75 0 01-1.5 0V4.75a.75.75 0 011.5 0v1.5z" />
                </svg>
                {error}
              </div>
            )}
            ...
            {step === 1 ? (
              <form onSubmit={handleVerify} className="flex flex-col gap-5">
                <div className="text-sm font-medium mb-1 text-slate-500 dark:text-slate-400">
                  Step 1: {isLibrarian ? 'Verify Librarian Access' : 'Verify Student Credentials'}
                </div>
              ...
              <form onSubmit={handleRegister} className="flex flex-col gap-5">
                {isLibrarian ? (
                  /* Librarians aren't on the roster, so there's no name to pull automatically */
                  <div>
